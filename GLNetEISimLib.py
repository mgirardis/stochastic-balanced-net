...
#pythran export RunSimulation_static(int, int, int, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, bool, int, str)
#pythran export GLNetEI_adapt_iter(float, float, float, float, float, float, float, float, float, float, float, float, float, float, float)
#pythran export GLNetEI_iter(float, float, float, float, float, float, float, float, float, float, float, float, float, float, float)
    ...
    if tTrans > 0:
        ...
        for t in range(1,tTrans):
            ...
            for i in range(pN):
                ...
                rhoE[t] = rhoE[t] + XE[i]
            for i in range(qN):
                VI[i],XI[i],dummyVar = GLNetEI_iter(VI[i],XI[i],rhoE[t-1],rhoI[t-1],Iext,mu,theta,J,Gamma,I,g,p,q,tauTinv,uT)
                rhoI[t] = rhoI[t] + XI[i]
            rhoE[t] = rhoE[t]/pN_fl
            rhoI[t] = rhoI[t]/qN_fl
        rhoETemp = rhoE[-1]
...
def RunSimulation_adapt(N,tTrans,Tmax,VE0,VE0Std,VI0,VI0Std,XE0,XE0Rand,XI0,XI0Rand,mu,theta,J,Gamma,I,Iext,g,p,q,A,tauWinv,uW,tauTinv,uT,saveSpikingData,nNeuronsSpk,weightDynType):
    ...
    if tTrans > 0:
        ...
        for t in range(1,tTrans):
            ...
            thetaMean = 0.0
            for i in range(pN):
                VE[i],XE[i],thetaE[i] = GLNetEI_adapt_iter(VE[i],XE[i],rhoE[t-1],rhoI[t-1],Iext,mu,thetaE[i],J,Gamma,I,W_I,p,q,tauTinv,uT)
                rhoE[t] = rhoE[t] + XE[i]
                thetaMean = thetaMean + thetaE[i]
            for i in range(qN):
                VI[i],XI[i],thetaI[i] = GLNetEI_adapt_iter(VI[i],XI[i],rhoE[t-1],rhoI[t-1],Iext,mu,thetaI[i],J,Gamma,I,W_I,p,q,tauTinv,uT)
                rhoI[t] = rhoI[t] + XI[i]
...
def RunSimulation_static(N,tTrans,Tmax,VE0,VE0Std,VI0,VI0Std,XE0,XE0Rand,XI0,XI0Rand,mu,theta,J,Gamma,I,Iext,g,p,q,A,tauWinv,uW,tauTinv,uT,saveSpikingData,nNeuronsSpk,weightDynType):
...
def GLNetEI_adapt_iter(V,X,rhoE,rhoI,Iext,mu,theta,J,Gamma,I,W_I,p,q,tauTinv,uT):
    theta = theta * (1.0 - tauTinv + uT*X)
    V = (mu*V + I + Iext + J*p*rhoE - q*W_I*rhoI)*(1.0-X)
    X = float(random.random() < PHI(V,theta,Gamma))
    return V,X,theta

def GLNetEI_iter(V,X,rhoE,rhoI,Iext,mu,theta,J,Gamma,I,g,p,q,tauTinv,uT):
