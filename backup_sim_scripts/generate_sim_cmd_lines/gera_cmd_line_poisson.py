...
gg = 4.0-5.0/(GG*J) # the critical point values for each Gamma in GG and J=10, essentially these values should not make any difference
...
#print(ttauw/ttau)
AA = numpy.unique(numpy.append(numpy.asarray((gg*J)*(1.0 + ttauw / ttau)),[30.0,40.0,50.0,60.0])) # eq 25 from the new paper
        ...
        for Y in YY:
            for N in NN:
                for tau in ttau:
                    ...
                    for A in AA:
                        if numpy.isnan(tauw_val):
                            parList = [G,g,Y,int(N),tau,A,rate1,rate2,tTotal]
                        else:
                            parList = [G,g,Y,int(N),tau,A,tauw,rate1,rate2,tTotal]
                        if runLocal:
                            if (kk%npar) == 0:
                                print('\n',end='')
